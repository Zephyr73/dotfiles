...
Panel {
  ...
  // Captive portal: a network (typically open) that gates internet access
  // behind a login page — e.g. campus "login required" WiFi. Quickshell
  // mirrors NetworkManager's connectivity state reactively, so this flips on
  // automatically the moment NM classifies the link as PORTAL.
  readonly property bool isPortal: networkManagerAvailable && Networking.connectivity === NetworkConnectivity.Portal
  ...
  // Open the captive-portal login page in a web browser. We point the browser
  // at a plain-HTTP site so the portal gateway is forced to reply with its
  // redirect; HTTPS-first sites can be silently blocked by a portal that only
  // intercepts port 80. The panel runs inside the user's graphical session,
  // so no extra display plumbing is required here.
  function openPortalLogin() {
    if (openBrowserProc.running) return
    openBrowserProc.command = ["firefox", "--new-window", "http://neverssl.com"]
  ...
  // Launch Firefox to the captive-portal login landing page.
  Process {
    id: openBrowserProc
  }
  ...
  KeyboardPanel {
    ...
    Column {
      ...
      // Captive portal login banner. Shown when NetworkManager classifies the
      // current link as a captive portal (e.g. campus "login required" WiFi):
      // the network is connected but your internet is gated behind a login
      // page. The button opens it in a browser.
      PanelSeparator {
        visible: root.isPortal
        foreground: root.bar.foreground
      }
      ...
      Column {
        ...
        Text {
          text: "This network requires login"
          color: root.bar.foreground
          font.family: root.bar.fontFamily
        ...
        Button {
          ...
          tooltipText: "Opens the login page in a web browser"
          foreground: root.bar.foreground
          fontFamily: root.bar.fontFamily
          bordered: true
          leftAlign: true
          width: parent.width
          onClicked: root.openPortalLogin()
        }
      }
